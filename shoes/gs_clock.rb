...
Shoes.app {
  ...
  def gen_recs(num)
    return stack {
		fill @fill
		num.times { |i| rect :left => @left, :top => (@top - (i*@offset)), :width => @width }
		@left += @offset
	}
  end
  ...
  def activate_images(stack, image_pos_array)
	stack.contents.each { |item| item.style :fill => @fill }
	image_pos_array.each { |pos| stack.contents[pos].style :fill => green }
  end
